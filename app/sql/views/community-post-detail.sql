...
SELECT
    ...
    COUNT(post_replies.reply_id) AS reply_count,
...
LEFT JOIN
    post_replies USING (post_id)
INNER JOIN
    profiles ON (profiles.profile_id = posts.profile_id)
GROUP BY
    posts.post_id, topics.topic_id, topics.name, topics.slug, profiles.profile_id, profiles.name, profiles.avatar, profiles.role, profiles.created_at;
